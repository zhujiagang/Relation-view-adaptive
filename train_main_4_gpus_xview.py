...
def get_parser():
    ...
    parser.add_argument(
        '--weights',
        default="./work_dir/NTU-RGB-D/xview/ST_GCN_dilation/epoch25_model.pt",
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label) in enumerate(loader):
            T_dim = data[:, -1, -1, -1, -1].numpy()
            num_seg = 25
            data = data[:, :, :-1, :, :]
            if self.arg.model_args['diff']:
                #### diff
                N, C, T, V, M = data.size()
                a1 = data[:, :, :-1]
                a2 = data[:, :, 1:]
                a2 = a2 - a1
                a22 = a2[:, :, -1].contiguous().view(N, C, 1, V, M)
                data = torch.cat((a2, a22), 2)

            xx = []
            for iiii in range(data.shape[0]):
                tt = T_dim[iiii]
                xxx = (tt - self.arg.model_args['T_dim']) / num_seg
                xxxx = []
                for ii in range(num_seg):
                    a1 = xxx * ii
                    a2 = int(xxx)
                    indice = int(a1 + random.randint(0, a2))
                    xxxx.append(data[iiii].unsqueeze(0)[:, :, indice:indice+self.arg.model_args['T_dim']])
                xxxx = torch.cat(xxxx,2)
                xx.append(xxxx)
            data = torch.cat(xx)
