...
class Feeder(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.random_view_transform = random_view_transform
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            # print (self.mode)
            data_temp_last = data_numpy[:, -1]
            data_temp_last = np.reshape(data_temp_last, (3,1,25,2))
            ...
            if self.random_view_transform and random.random() > 0.5:
                data_numpy = random_view_transform(data_numpy)
            data_numpy = np.concatenate((data_numpy, data_temp_last), axis=1)
        # mean subtraction
