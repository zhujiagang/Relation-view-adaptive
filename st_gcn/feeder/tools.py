...
def random_move(
    ...
    theta = np.array([[np.cos(a) * s, -np.sin(a) * s],
                        [np.sin(a) * s, np.cos(a) * s]])###2*2*300

    # perform transformation
    for i_frame in range(T):
        xy = data_numpy[0:2, i_frame, :, :]###2*25*2
        new_xy = np.dot(theta[:,:,i_frame], xy.reshape(2, -1))###2*50
...
def random_view_transform(X, angle1=-10, angle2=10, s1=0.9, s2=1.1):
    ...
    # xx = np.dot(Rx,Ss)###3*3
    # xxx = np.dot(Ry,np.dot(Rx,Ss))###3*3
